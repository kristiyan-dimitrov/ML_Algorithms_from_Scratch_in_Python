...
class GMM():
    ...
    def fit(self, features):
        ...
        print(f"INITIALIZED COVARIANCES {self._init_covariance(features.shape[-1])}")
    ...
    def _m_step(self, features, assignments):
        ...
        for j in range(self.n_clusters):
            mixing_weights[j] = sum(assignments[:,j]) / features.shape[0]
            # mixing_weights[j] = sum(sum(assignments[j,:])) / features.shape[0]


        # MEANS
        # means = np.zeros(self.n_clusters)
        means = np.zeros((self.n_clusters, features.shape[-1])) # Alternative for means

        for j in range(self.n_clusters):
            # I think all this is a bit "1-dimensional"
            means[j] = sum([assignments[i,j]*features[i,:] for i in range(features.shape[0])])  / sum([assignments[i,j] for i in range(features.shape[0])])
        ...
        if self.covariance_type == 'spherical':
            ...
            print(f"INITIAL COVARIANCES: {covariances} <@&@&@&@&@---------------------")
            for j in range(self.n_clusters):
                
                print(f"SAMPLE NUMERATOR i=10, j=0: { assignments[10,0]*((features[10,:]-means[0])**2)}")


                covariances[j] = sum([assignments[i,j]*((features[i,:]-means[j])**2) for i in range(features.shape[0])]) / sum([assignments[i,j] for i in range(features.shape[0])])
