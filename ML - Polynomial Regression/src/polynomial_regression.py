...
class PolynomialRegression():
    ...
    def visualize(self, features, targets):
        ...
        # Calculate feature weights
        w = hat_matrix.dot(targets)

        # Calculating predictions
        predictions = X.dot(w)

        # Need to sort the features, predictions, and targets so the fit plot comes out properly
        tpl_lst = list(zip(features, predictions, targets))
        srtd_tpl_lst = sorted(tpl_lst, key=lambda x: x[0])
        features, predictions, targets = zip(*srtd_tpl_lst)

        plt.scatter(features, targets)
        plt.plot(features, predictions, "-")
        plt.title(f'degree{degree}')
        plt.savefig(f'degree_{self.degree}')
