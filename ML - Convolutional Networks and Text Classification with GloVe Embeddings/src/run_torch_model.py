...
# Doing my best for reproducibility
torch.manual_seed(42)
np.random.seed(42)

def run_model(model,running_mode='train', train_set=None, valid_set=None, test_set=None,
    batch_size=1, learning_rate=0.01, n_epochs=1, stop_thr=1e-4, shuffle=True, label_dict=None):
    """
    ...
    if running_mode == 'train':
        ...
        for epoch in range(n_epochs):
            ...
            print("EPOCH ", epoch)
            model, epoch_train_loss, epoch_train_acc = _train(model, train_dataloader, solver, device, label_dict)
    ...
    if running_mode == 'test':
        ...
        loss, accuracy, predictions = _test(model, test_dataloader, device)

        return loss, accuracy, predictions


def _train(model,data_loader,optimizer,device=torch.device('cpu'), label_dict=None):
    ...
    for batch, labels in data_loader: # Per this doc, (batch,labels) = trainloader.__getitem__(); https://pytorch.org/docs/stable/torchvision/datasets.html#cifar
        ...
        # transformed_predictions = [label_dict[prediction.item()] for prediction in predicted]
